        ...
        auto do_eval = [&](const Value & l) -> Value {
          if (op == Op::Is || op == Op::IsNot) {
            ...
            auto eval = [&]() {
              ...
              // r4dx local patch (see third_party/VERSIONS.md): minja represents an undefined
              // Jinja variable as a null Value (consistent with "defined" above testing
              // `!l.is_null()`), but had no complementary "undefined" test -- needed because
              // Qwen3.8-27B's own chat_template.jinja uses `... is undefined` directly (e.g.
              // `{%- if enable_thinking is undefined or enable_thinking is true %}`), which
              // otherwise throws "Unknown type for 'is' operator: undefined" on every render.
